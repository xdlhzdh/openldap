...
class LDAPServer:
    def __init__(self):
        ...
        self.server_socket.bind(("127.0.0.1", 1234))
        # Get the port number
                ...
                for sock in readable:
                    if sock is self.server_socket:
                        ...
                        if ber_req_times == 1:
                            continue
                        try:
                            received_data, remaining_len = self.recv_ldap_message(
                                sock, length
                            )
                            length = remaining_len
                            if received_data is not None and len(received_data) > 0:
                                self.request_data += received_data
                                print(
                                    f"received_data length = {len(received_data)}, remaining data length = {remaining_len}, \
request_data length = {len(self.request_data)}"
                                )
                            if remaining_len == 0 and len(self.request_data) > 0:
                                print(f"start decode: {self.request_data}")
                                ldap_message, _ = decoder.decode(
                                    self.request_data, asn1Spec=LDAPMessage()
                                )
                                ber_req_times = ber_req_times + 1
                                print(f"ber_req_times = {ber_req_times}")
                                print(ldap_message.prettyPrint())
                                # Check if it is a StartTLS request
                                message_id = int(
                                    ldap_message.getComponentByName("messageID")
                                )
                                protocol_op = ldap_message.getComponentByName(
                                    "protocolOp"
                                )
                                extended_req = protocol_op.getComponentByName(
                                    "extendedReq"
                                )
                                request_name = extended_req.getComponentByName(
                                    "requestName"
                                )
    ...
    def construct_extended_response(
        self, message_id, result_code, response_name=None, response_value=None
    ):
    ...
    def recv_ldap_message(self, sock: socket.socket, length=None):
        ...
        if length is None:
            ...
            if first_length_byte & 0x80 == 0:  # Short form
                ...
                )  # Get the number of bytes representing the length
